...
export const SocketContextProvider = ({
  ...
  const [ongoingCall, setOngoingCall] = useState<null | OngoingCall>(null);
  ...
  const handleCall = useCallback(
    async (user: SocketUser) => {
      ...
      if (!stream) {
        console.log("No stream handle call");
        return;
      }
  ...
  const onIncomingCall = useCallback(
    (participants: Participants) => {
      ...
      console.log("ongoingCall at 101",ongoingCall)
    },
  ...
  const completePeerConnection = useCallback(
    async (connectionData: {
      ...
      setPeer({
        ...
        participantUser: ongoingCall?.participants.receiver as SocketUser,
        stream: undefined,
      });
  ...
  const handleJoinCall = useCallback(async(ongoingCall : OngoingCall) => {
    ...
    if (!stream) {
      console.log("could not get stream in handleCallJoin");
    ...
    setPeer({
      peerConnection: newPeer,
      participantUser: ongoingCall?.participants.caller as SocketUser,
      stream: undefined,
    });
  ...
  }, [socket, isSocketConnect, user, onIncomingCall,completePeerConnection]);
